      ...
      <v-col cols="12">
        <v-card>
            ...
            >
                <div class="fill-height gradient-overlay d-flex align-end">
                    <div class="pa-4 w-100">
                        <div class="text-subtitle-1 text-white font-weight-bold opacity-80 mb-1" v-if="routeDetail.cave_system">
            ...
            <div v-else class="pa-4">
                <div class="text-subtitle-1 text-medium-emphasis font-weight-bold mb-1" v-if="routeDetail.cave_system">
                     {{ routeDetail.cave_system.name }}
                </div>
                ...
                <v-row class="mt-2">
                    <v-col cols="12" md="8">
                        <h3 class="text-h6 mb-2">Details</h3>
                         ...
                         <div class="mt-4 text-body-1" v-if="routeDetail.description">
                        ...
                        <div v-if="routeDetail.media && routeDetail.media.length > 0">
                            ...
                            <v-row dense>
                                <v-col v-for="media in routeDetail.media" :key="media.id" cols="12" sm="6">
                                    <v-card @click="openMedia(media)" link>
                                        <v-img 
                                            v-if="media.type !== 'pdf'"
                                            :src="media.path" 
                                            height="150" 
                                        ...
                                        <v-card-subtitle class="py-2 text-caption" v-if="media.caption">{{ media.caption }}</v-card-subtitle>
                                        <v-card-subtitle class="py-2 text-caption" v-else>{{ media.type === 'pdf' ? 'View PDF' : 'View Image' }}</v-card-subtitle>
                                    </v-card>
                                </v-col>
                            </v-row>
                        </div>
