    ...
    >
      ...
      <div class="position-absolute top-0 left-0 w-100 p-4 d-flex align-center pa-4">
        ...
        <template v-if="currentUserWasOnTrip">
          <v-btn 
            ...
            variant="tonal" 
            color="white" 
            class="mr-2 backdrop-blur"
            @click="$router.push({name: '/trip/[id].edit', params: {id: trip.id}})" 
          ></v-btn>
          <v-btn 
            icon="mdi-delete" 
            variant="tonal" 
            color="error" 
            class="backdrop-blur text-white"
      ...
      <v-container class="pb-6">
        ...
        <div class="d-flex align-center text-white">
          ...
          <v-chip 
            :color="getVisibilityColor(trip.visibility)" 
            size="small" 
            variant="flat"
          >
                    ...
                    <v-hover v-slot="{ isHovering, props }">
                      ...
                      >
                         <v-img
                           ...
                           aspect-ratio="1"
                           cover
                           class="bg-grey-lighten-2 transition-transform"
          ...
          <v-card class="rounded-lg mb-6" elevation="2">
             ...
             <v-list class="py-0">
                ...
                <v-list-item class="py-3">
                   ...
                   <v-list-item-title>
                      <router-link :to="{name: '/cave/[id]', params: {id: trip.entrance.slug}}" class="text-decoration-none font-weight-bold text-high-emphasis text-primary">
                         {{ trip.entrance.name }}
                      </router-link>
                   </v-list-item-title>
                </v-list-item>
                ...
                <v-list-item class="py-3" v-if="trip.exit && trip.entrance.id !== trip.exit.id">
                   ...
                   <v-list-item-title>
                      <router-link :to="{name: '/cave/[id]', params: {id: trip.exit.slug}}" class="text-decoration-none font-weight-bold text-high-emphasis text-primary">
                         {{ trip.exit.name }}
                      </router-link>
                   </v-list-item-title>
                </v-list-item>
                ...
                <v-list-item class="py-3">
                   ...
                   <v-list-item-title class="font-weight-medium">{{ formatTime(trip.start_time) }}</v-list-item-title>
                </v-list-item>
                ...
                <v-list-item class="py-3">
                   ...
                   <v-list-item-title class="font-weight-medium">{{ formatDuration(trip.start_time, trip.end_time) }}</v-list-item-title>
                </v-list-item>
             </v-list>
          </v-card>
          ...
          <v-card class="rounded-lg" elevation="2">
             ...
             <v-list class="py-2">
                <v-list-item
                  ...
                  :key="participant.id"
                  :to="{ name: '/profile/[id]', params: { id: participant.id } }"
                  rounded="lg"
                  class="ma-2 mb-1"
                >
                  <template v-slot:prepend>
                     <v-avatar color="grey-lighten-2" size="40" class="border">
                        <v-img :src="participant.photo || '/default-avatar.png'" :alt="participant.name" cover></v-img>
                     </v-avatar>
                  </template>
                  <v-list-item-title class="font-weight-bold">{{ participant.name }}</v-list-item-title>
                  <v-list-item-subtitle class="text-caption text-truncate">
                     {{ participant.clubs && participant.clubs.length ? participant.clubs.map(c => c.name).join(', ') : 'No club' }}
