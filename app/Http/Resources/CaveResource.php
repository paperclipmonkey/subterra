...
class CaveResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'tags' => TagResource::collection($this->tags->merge(collect($previoslyDoneTag ? [$previoslyDoneTag] : [])->merge($systemLengthTags))),
            ...
            'system' => $this->system ? [
                ...
                'files' => $this->system->files->map(function ($file) {
                    return [
                        'id' => $file->id,
                        'url' => Storage::disk('media')->url("cave_system_files/{$this->cave_system_id}/{$file->filename}"),
